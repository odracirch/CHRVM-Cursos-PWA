...
export default function Login() {
  ...
  async function go(e: React.FormEvent) {
    ...
    if (data.session) {
      localStorage.setItem('chrvm_access', data.session.access_token)
      localStorage.setItem('chrvm_refresh', data.session.refresh_token)
    }
    ...
    <div className="max-w-md mx-auto px-4 py-14">
      <div className="card p-7">
        ...
        <p className="text-sm mt-5">
          ¿No tienes cuenta?{' '}
          <Link className="text-brand-600 font-semibold" href="/registro">
            Regístrate
          </Link>
        </p>
      </div>
